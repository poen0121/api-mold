 ...
 * Class IndexCriteria.
 *
 * @package namespace App\Criteria\Service\Client;
 */
class IndexCriteria implements CriteriaInterface
{
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        ...
        /* Name fuzzy query */
        $nameQuery = $this->getFiller('name');
        if (isset($nameQuery)) {
            if (isset($quest)) {
                $quest = $quest->where('name', 'LIKE', '%' . addcslashes($nameQuery, '%_\\') . '%');
            } else {
                $quest = $model->where('name', 'LIKE', '%' . addcslashes($nameQuery, '%_\\') . '%');
            }
        }
        /* Order by created_at */
