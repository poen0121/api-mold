...
class BetterApiDocWriter extends Writer
{
    ...
    public function generateMarkdownOutputForEachRoute(Collection $parsedRoutes, array $settings): Collection
    {
        $parsedRouteOutput = $parsedRoutes->map(function (Collection $routeGroup) use ($settings) {
            return $routeGroup->map(function (array $route) use ($settings) {
                if (count($route['cleanBodyParameters']) && ! isset($route['headers']['Content-Type']) && (in_array('POST', $route['methods']) || in_array('PUT', $route['methods']) || in_array('PATCH', $route['methods']))) {
                    // Set content type if the user forgot to set it
                    $route['headers']['Content-Type'] = 'application/x-www-form-urlencoded';
                }
    ...
    {
        $prependFile = 'resources/views/apidoc/prepend.md';
    ...
    {
        $appendFile = 'resources/views/apidoc/append.md';
